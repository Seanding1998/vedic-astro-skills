...
| 独立智能体断后校验 | 已支持 | 只能由新启动、无历史上下文子代理执行的两阶段证据账本审查；固定 PASS/REVISE 门槛 |
...
| 交付前审查 | 各宿主可自行决定如何审核模块产物 | 本分支强制作者与审查分离：新启动、无历史上下文代理先独立形成证据账本，再给出 PASS/REVISE；作者不能审自己的结论 |
...
个人化结论、精确数值或报告交付
  -> 数据校验完成，形成候选答案
  -> 新启动的干净上下文代理先建立独立证据账本
  -> 再接收候选答案，输出 PASS / REVISE
  -> 非 PASS、超时或上下文泄漏均阻断交付；重大修订换新代理复审
```
...
本分支把“断后校验”做成了交付前的独立质量门，而不是作者给自己答案的润色。命盘解读、专题结论、时间窗口、报告和精确数值必须先通过一个**新启动、不继承起草对话历史**的审查代理。

流程分两段：审查代理先只看到原始问题、适用规则、允许背景和已验证数据，独立形成“证据能支持什么主张”的账本；候选答案只会在账本封存后交给它对照。这样不会让候选措辞提前锚定审查结论，也不会把起草代理的隐藏推理、内部路由、自我评价或中间稿泄漏给审查者。

审查输出固定的机器可读 `PASS` / `REVISE` verdict。**这项审查只能由新启动、干净上下文子代理执行；起草代理的自检、同一对话代理的复核或未审查标记都不能替代。** 超时、无法启动新子代理、上下文泄漏、审查包不完整、格式错误、`REVISE`，或缺少 `release_allowed: true` 都会阻断触发审查的解释性结论、时间窗口、报告和数值判断。改动结论、数值、时间、证据或置信度后，必须换另一名新的干净上下文代理重新审查。这个门检查证据支持、规则遵守、范围和不确定性处理，不宣称能证明外部世界中的占星结论必然为真。

完整协议见 [references/审查与断后校验.md](references/审查与断后校验.md)。纯计算、提取、schema 校验和补充信息提问可豁免；一旦同一回复包含解释性结论，就必须进入审查门。
