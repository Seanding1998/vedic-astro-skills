...
| 独立智能体断后校验 | 已支持 | 新启动、无历史上下文代理的两阶段证据账本审查；固定 PASS/REVISE 门槛 |
...
审查输出固定的机器可读 `PASS` / `REVISE` verdict。超时、上下文泄漏、审查包不完整、格式错误、`REVISE`，或缺少 `release_allowed: true` 都会阻断交付。改动结论、数值、时间、证据或置信度后，必须换另一名新的干净上下文代理重新审查。这个门检查证据支持、规则遵守、范围和不确定性处理，不宣称能证明外部世界中的占星结论必然为真。
