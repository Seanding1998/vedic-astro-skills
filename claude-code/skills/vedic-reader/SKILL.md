...
时间事件专项规则：
  ...
  2. ⚠️ 四通道分析（不要只看通道1！）
     AD主星触发事件有4个通道，必须全部扫描：
     
     通道1: 宫主身份 — AD主星管哪些宫？ 
            例：Mercury管L7+L10 → 位移/事业
     通道2: 落宫激活 — AD主星坐在哪个宫？该宫主题被激活
            例：Rahu坐7宫 → 7宫=位移/旅行被激活 ✅
     通道3: 天然象征 — AD主星的naisargika karaka本性
            例：Rahu天然代表异地/打破常规 → 搬迁信号 ✅
     通道4: Chara Karaka — AD主星的8K身份
            例：DK的小运 → 配偶相关事件
     
     ⚠️ 历史教训（xiaobo盘）：
       搬迁 → 只看通道1(L4=Jupiter)选错了Sat-Jup小运
              实际触发是Sat-Rahu，通过通道2(Rahu坐7宫=位移)
              + 通道3(Rahu天然异地象征)触发
       事业 → 只看通道1(L10)选了Merc-Rahu
              实际触发是Merc-Mars，通过通道1(L12=阶段结束)
              + 通道2(Mars坐12宫own sign=主动终结)触发
     
     正确做法：对每个候选AD，四通道都扫一遍，
     多通道同时指向同一事件类型 = 强信号
     只有1个通道 = 弱信号，除非该通道信号极强
  ...
  5. 按事件类型选AD（四通道综合，不再只看宫主）：
     搬家 → 查L4/L12的小运(通道1) + 坐4/7/12宫的星(通道2) + Rahu/Ketu(通道3)
     工作 → 查L10/L6的小运(通道1) + 坐10宫的星(通道2)
     升学 → 查L4/L5的小运(通道1) + Jupiter/Mercury(通道3)
     感情 → 查L7的小运(通道1) + DK(通道4) + Venus(通道3)
     突变 → 查L8的小运(通道1) + 坐8宫的星(通道2)
