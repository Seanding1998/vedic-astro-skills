...
2. **执行硬约束** → 不可降级的底线
3. **独立智能体审查（断后校验）** → 个人化结论在交付前必须由干净上下文子代理审查
4. **主动路由** → 匹配 reference，匹配后**必须读取对应 reference 全文**
5. **信号分级与篇幅控制** → 决定每颗行星写多深
6. **输出风格硬约束** → 影响说话方式但不改变框架
...
“层 3：事后校验”用于诊断推断链；本节用于**交付前**阻断无证据、越界或未降级的候选答案。两者不能互相替代。只要交付个人化解读、时间窗口、报告或精确数值，必须在发送前读取并执行 `references/审查与断后校验.md`。

这不是恢复旧“盲审”体系：层 1 背景事实仍是合法输入。隔离的是起草代理的历史、隐藏推理、内部路由和自我评价，避免作者为自己的结论背书。

强制流程：

1. 起草代理完成数据校验并形成候选答案，但不得发送。
2. 以不继承对话历史的方式启动新的审查子代理（与 `fork_context=false` 等价）。
3. Phase A：审查代理只基于原始问题、适用规则、允许背景和已验证事实包，先形成独立证据账本；此时禁止看到候选答案。
4. Phase B：Phase A 封存后才交付候选答案，审查代理按协议输出机器可读的 `PASS` / `REVISE` verdict。
5. 审查超时、隔离失败、审查包不完整、verdict 不合法、`REVISE` 或没有 `release_allowed: true` 时 fail closed：不得交付。
6. 修订若影响结论、数值、时间、证据或置信度，必须换**另一名**新的干净上下文审查代理重新执行两阶段审查。

起草代理绝不能审查、批准、豁免或重新解释自己的候选答案。**断后校验的唯一合法执行器是新启动、干净上下文的审查子代理。** 不能启动该子代理时，审查链失败：不得发送任何触发审查的解释性结论、时间窗口、报告或数值判断；只能说明当前宿主无法执行独立审查，或输出本协议明确豁免的纯机械结果。审查代理不重写整篇答案，只检查证据可追溯性、数值校验、不确定性、结构/窗口/专名边界、高风险表述和最低交付契约。
...
| 交付前的个人化结论、数值结论或报告 | `references/审查与断后校验.md` | 两阶段、干净上下文审查代理的 fail-closed PASS/REVISE 协议 |
